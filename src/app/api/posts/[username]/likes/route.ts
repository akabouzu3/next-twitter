...
export async function GET(request: NextRequest, { params }: Props) {
  try {
    ...
    /**
     * query string取得
     * URL: /api/posts/[username]/likes?limit=10&cursor=xxx
     */
     ...
     * - 値あり → 次ページ
     */
    const page = await getUserLikedPostFeedPageByUserId({
      userId: user.id,
      limit,
      cursor,
    });
     ...
     * 想定外エラー（DBエラーなど）
     */
    console.error("[GET /api/posts/[username]/likes]", error);

    return NextResponse.json(
      {
        message: "ユーザがいいねした投稿の取得に失敗しました",
      },
      { status: 500 },
    );
  }
